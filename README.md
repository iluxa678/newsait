...
В городах Российской Федерации проживает около 75% населения [[1](#1)]. Большинство содержат в домашних условиях собак, кошек, птиц, рептилий и т.д. В частных секторах ещё и сельскохозяйственных животных, таких как коровы, лошади, сельскохозяйственная птица, а так же малый рогатый скот.
...
Для сравнения были рассмотренны 4 системы автоматизации ветеринарно клиники [[5](#5)].
...
В процессе проектирования системы, изначально была разработанна диаграмма вариантов использования (см. рисунок 1). В результате анализа предметной области было определено 4 сущности: клиент, оператор, вет. диетолог и вет. врач. 
...
Далее была разработанна диаграмма потоков данных (см. рисунок 2). В результате анализа предметной области были определены основные сущности данных и действия, требующие взаимодействия с этими сущностями.
...
По представленным диаграммам были разработанны следующие классы (см. рисунок 4). 
...
Так же по представленным диаграммам были разработанны хранилища, которые позволяют создавать, хранить, актуализировать и использовать информацию в системе (см. рисунок 5).
...
Листинг 3 - Хранилище класса "Обращение"
...
Листинг 4 - Хранилище класса "Клиент"
...
Так же по представленным диаграммам были разработанны контроллеры, применяемые для взаимодействия с клиентом (см. рисунок 6).
...
Листинг 6 - Контроллер "Вет. диетолога"
...
После запуска программы открывается страница Swagger UI со списком контроллеров и их операциями над сущностями (см. рисунок 7).
...
1) Create позволяет добавлять новую запись в хранилище (см. рисунок 9).
<p align="center">
...
После успешного выполнения операции сервер выдаёт ответ (см. рисунок 10).
<p align="center">
...
2) Функция чтения Read похожа на функцию поиска, она позволяет извлекать определенные записи и считывать их значения (см. рисунок 11).
<p align="center">
...
После выполнения операции сервер выдаёт информацию по записи, в которой содержался данный идентификатор (см. рисунок 12).
<p align="center">
...
3) Функция редактирования данных Update(см. рисунок 13). Используется для изменения существующих записей в базе данных. Находит запись по идентификатору, подобно функции Read.
<p align="center">
<img src=https://user-images.githubusercontent.com/91088733/146267499-f6a3b378-e37d-4c12-a596-0f3ca820887b.png></p>
...
<p align="center">
<img src=https://user-images.githubusercontent.com/91088733/146267710-8e2a88c4-3dad-4127-ae0c-91ca6f3b18a0.png></p>
<p align="center">Рисунок 14 - Результат выполнения метода Update</p> 

Функцией Read проверим вышеуказанную операцию. После срабатывания сервер выдает новую информацию(см. рисунок 15).
<p align="center">
<img src=https://user-images.githubusercontent.com/91088733/146267977-0454b7d9-dfdb-4da9-9d43-5f5167b7c72f.png></p>
<p align="center">Рисунок 15 - Результат выполнения операции Read после обновления данных</p> 

4) Delete - используется для удаления записи по идентификатору. Если операция удаления сработает успешно, то ответ сервера будет "true", иначе "false" (см. рисунок 16).
<p align="center">
<img src=https://user-images.githubusercontent.com/91088733/146268233-8d19ac15-26ba-4e68-8d72-93f34b51b8a3.png></p>
<p align="center">Рисунок 16 - Окно метода удаления с введенным идентификатором</p> 

После выполнения операции ответ сервера (см. рисунок 17)
<p align="center">
<img src=https://user-images.githubusercontent.com/91088733/146268480-d9bc699e-cb3c-468e-9a99-e864bdbef76b.png></p>
<p align="center">Рисунок 17 - Результат выполнения метода Delete</p> 
...
В результате проделанной работы были достигнуты поставленные цели и задачи проекта. В процессе проектирования удалось реализовать и протестировать информационную систему автоматизации "Система ветпомощи нуждающимся". Данная информационная система выполняет функции создания, чтения, редактирования и удаления записей. Проблема, поставленная в начале работы, была решена в процессе разработки информационной системы.
...
<a name="1"/>1. Население России. Wikipedia [Электронный ресурс]. – Режим доступа: https://ru.wikipedia.org/wiki/%D0%9D%D0%B0%D1%81%D0%B5%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5_%D0%A0%D0%BE%D1%81%D1%81%D0%B8%D0%B8, свободный (дата обращения: 30.09.2021).

<a name="2"/>2. Профессия диетолог. Enjoy-job [Электронный ресурс]. – Режим доступа: https://enjoy-job.ru/professions/dietolog/, свободный (дата обращения: 20.10.2021).

<a name="3"/>3. Что лечит врач-терапевт? Docdoc [Электронный ресурс]. – Режим доступа: https://docdoc.ru/library/terapevtiya/chto-lechit-vrach-terapevt, свободный (дата доступа: 5.11.2021).

<a name="4"/>4. Цели, задачи и предмет деятельности учреждений ветеринарии. Chelagro [Электронный ресурс]. – Режим доступа: http://www.chelagro.ru/about/subordinated/goals_and_objectives_of_veterinary_institutions.php, свободный (дата доступа: 05.11.2021).

<a name="5"/>5. Топ 10: Программы для ветеринарных клиник. Livemedical [Электронный ресурс]. – Режим доступа: https://www.livemedical.ru/tools/vet/, свободный (дата доступа: 07.11.2021).
